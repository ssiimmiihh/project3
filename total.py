...
NAVER_CLIENT_ID = "8M7qNQSzjXj7bSOkm3Ig"
NAVER_CLIENT_SECRET = "QU3VU_3J6Y"
...
mozza_questions = [
    "가성비 있는 모짜렐라 치즈 추천해주세요.",
    "모짜렐라 치즈 브랜드 비교해주세요.",
    "긍정적인 리뷰가 가장 많은 제품은 무엇인가요?"
]
default_queries_map = {
    "쇼핑": mozza_questions[0], # 쇼핑 탭 기본 질문
    "블로그": "아사이볼 토핑 추천",
    "뉴스": "최근 세계적인 이슈는 무엇인가요?"
}
...
# 검색 입력 필드 도움말 텍스트
help_texts = {
    "쇼핑": "모짜렐라 치즈 추천 질문을 클릭하거나 직접 검색어를 입력하세요.",
    "블로그": "블로그 관련 검색어를 입력하세요. (예: 아사이볼 맛집)",
...
if active_source_type == "쇼핑":
    st.markdown("👇 **모짜렐라 치즈 관련 질문을 선택해보세요!**")
    cols = st.columns(len(mozza_questions))
    for i, q_text in enumerate(mozza_questions):
        if cols[i].button(q_text, key=f"mozza_q_btn_{i}"):
